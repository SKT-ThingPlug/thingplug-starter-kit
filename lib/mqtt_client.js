...
var MQTTClient = function(config){
  var options = {
    clientId : config.nodeID + '_' + new Date().getTime()
  ...
  client.on('message', function(topic, message){
    ...
    if (topic.indexOf('/req/')>0) {
      self.emit('command', topic, message);
    }
  ...
  this.send = function(payload){
    var topic = util.format('/oneM2M/req/%s/%s', config.nodeID, config.appEUI);
  ...
  this.createNode = function(cb){
    ...
    self.send(payload);
    self.on('resp', function(topic,res){
      if(res.indexOf(ri) > 0){
        xml2js.parseString(res, function(err, xmlObj){
          if(err) return self.emit('error', err);
          if((xmlObj['m2m:rsp']['rsc'][0] === statusCode.OK) ||
             (xmlObj['m2m:rsp']['rsc'][0] === statusCode.EXIST)) {
  ...
  this.createRemoteCSE = function(cb){
    ...
    var reqBody = {
     ...
     ,'cty': 'application/vnd.onem2m-prsp+xml'
     ,'nm' : config.nodeID
     ,'pc' : "<csr><cst>3</cst><csi>"+config.nodeID+"</csi><rr>true</rr><nl>"+config.nodeRI+"</nl></csr>"
    }
    var payload = getXMLRoot().ele(reqBody).end();
    self.send(payload);
    self.on('resp', function(topic,res){
